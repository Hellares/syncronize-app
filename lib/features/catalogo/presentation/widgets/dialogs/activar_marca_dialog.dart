...
/// Diálogo para activar una marca maestra
class ActivarMarcaDialog extends StatefulWidget {
  final MarcaMaestra maestra;
  final String empresaId;

  const ActivarMarcaDialog({
    super.key,
  ...
  @override
  State<ActivarMarcaDialog> createState() => _ActivarMarcaDialogState();
}

class _ActivarMarcaDialogState extends State<ActivarMarcaDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Activar Marca'),
      content: SingleChildScrollView(
        child: Form(
          ...
          child: Column(
            ...
            children: [
              // Información de la marca maestra
              Container(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: _isLoading ? null : _activarMarca,
          child: _isLoading
              ? const SizedBox(
                  width: 16,
                  height: 16,
  ...
  Future<void> _activarMarca() async {
    ...
    final result = await cubit.activarMarca(
      ...
      marcaMaestraId: widget.maestra.id,
